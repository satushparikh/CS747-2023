...
def task2(algorithm, horizon, p1s, p2s, num_sims=50):
    ...
    for prob in probs:
        ...
        to_append = (simulate_task2(algorithm, prob, horizon, num_sims))
        regrets.append(to_append)
        # print(to_append)
    return regrets
...
if __name__ == '__main__':
  ...
  # TASK A
  p1=0.9
  p2s=np.arange(0,0.91,0.05)
  # p2s=np.arange(0,0.11,0.05)
  regrets = task2(UCB,30000,[p1]*len(p2s),p2s,num_sims=50)
  plt.plot(p2s,regrets,marker='o',linestyle='-')
  plt.xlabel("p2 (Mean of Second Arm)")
  ...
  # for task B
  p2B = np.arange(0.00, 0.95, 0.05)
  p1B = p2B + 0.1

  regret_ucb = task2(UCB, 30000, p1B, p2B, num_sims=50)
  regret_klucb = task2(KL_UCB, 30000, p1B, p2B, num_sims=50)

# Create the first figure and plot the UCB data
  plt.figure()  # Create a new figure
  plt.xlabel("p2 (Mean of Second Arm)")
  plt.ylabel("Average Regret")
  plt.title("Difference between means of arms constant=0.1")
  plt.plot(p2B, regret_ucb, 'bo-', label="UCB")
  plt.legend()
  plt.show()

# Create the second figure and plot the KL-UCB data
  plt.figure()  # Create another new figure
  plt.xlabel("p2 (Mean of Second Arm)")
  plt.ylabel("Average Regret")
  plt.title("Difference between means of arms constant=0.1")
  plt.plot(p2B, regret_klucb, 'go-', label="KLUCB")
  plt.legend()
  plt.grid(True)
  plt.show()
